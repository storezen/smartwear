      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {data.comparePrice && (
              <span className="text-foreground/30 text-[10px] line-through">Rs. {data.comparePrice.toLocaleString()}</span>
            )}
          </div>
    ...
    <>
      {!isOpen && (
        <motion.button
          ...
          className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-[#25D366] shadow-[0_8px_32px_rgba(37,211,102,0.35)] flex items-center justify-center text-foreground hover:bg-[#20BD5A] transition-colors"
        >
          <MessageCircle className="w-6 h-6" />
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 text-foreground text-[10px] font-bold flex items-center justify-center shadow-lg">
              {unreadCount}
            </span>
          )}
        </motion.button>
          ...
          >
            ...
            <div className="bg-[#075E54] px-4 py-3 flex items-center justify-between cursor-pointer" onClick={() => setIsMinimized(!isMinimized)}>
              <div className="flex items-center gap-3">
                <div className="relative w-9 h-9 rounded-full bg-[#25D366]/20 flex items-center justify-center text-foreground text-sm font-bold shrink-0">
                  S
                    ...
                    <div className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`max-w-[88%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${
                          msg.role === "user"
                            ? "bg-[#075E54] text-foreground rounded-br-sm"
                            : "bg-[#1F2A33] text-foreground/& rounded-bl-sm"
                        }`}
                      ...
                      >
                        ...
                        <div className="flex items-center justify-end gap-1 mt-1">
                          <span className="text-[10px] text-foreground/40">{formatTime(msg.timestamp)}</span>
                    ...
                    {msg.role === "assistant" && msg.id !== "welcome" && (
                      <div className="flex items-center gap-2 mt-0.5 ml-2">
                        <button
                          ...
                          className={`p-1 rounded transition-colors ${msg.feedback === 1 ? "text-emerald-400" : "text-foreground/20 hover:text-foreground/50"}`}
                        >
                          <ThumbsUp className="w-3 h-3" />
                        </button>
                        <button
                          onClick={() => sendFeedback(msg.id, -1)}
                          className={`p-1 rounded transition-colors ${msg.feedback === -1 ? "text-red-400" : "text-foreground/20 hover:text-foreground/50"}`}
                        >
                    ...
                    <div className="bg-[#B8860B]/10 border border-[#B8860B]/20 rounded-xl px-4 py-3 text-center max-w-[90%]">
                      ...
                      <div className="flex items-center gap-2 justify-center">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#25D366] text-foreground text-xs font-semibold hover:bg-[#20BD5A] transition-colors"
                        >
              ...
              <div className="border-t border-border bg-background">
                  ...
                  <div className="px-3 pt-2 pb-1 flex flex-wrap gap-1.5">
                    {quickReplies.map((qr) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full bg-card border border-border text-foreground/60 text-[11px] hover:bg-card hover:text-foreground/& transition-all shrink-0"
                      >
                        {qr}
                      </button>
                    ))}
                ...
                <form onSubmit={handleSubmit} className="flex items-center gap-2 px-3 py-2.5">
                  ...
                  <button
                    ...
                    className="w-10 h-10 rounded-full bg-[#25D366] flex items-center justify-center text-foreground disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#20BD5A] transition-colors shrink-0"
                  >
