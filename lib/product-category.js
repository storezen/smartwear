...
function isAudioProduct(searchStr) {
  return /\b(airpods?|earbuds?|earpods|galaxy buds|tws|headphones)\b/i.test(searchStr);
}
...
/** Priority 3 — camera lens protectors (standalone lens protection, not full phone cases). */
function isCameraProtector(title, searchStr) {
  if (/\b(camera lens|lens protector|camera protector)\b/i.test(searchStr)) {
    return true;
  }

  // Lens protector covers sold explicitly for iPhone camera modules
  if (
    /\b(lens|camera)\b/i.test(searchStr) &&
    /\b(protector|protection|cover|tempered glass)\b/i.test(searchStr) &&
    /\b(iphone|pro max|pro)\b/i.test(searchStr) &&
    !/\b(silicon case|acrylic case|phone case)\b/i.test(title.toLowerCase())
  ) {
    return true;
  }
...
function getProductCategory(title = '', tags = '', type = '', handle = '') {
  ...
  if (isCameraProtector(safeTitle, searchStr)) {
