/**
 * Product categorization into 10 final store categories.
 *
 ...
 *   5. Analog Watches
 *   6. Watch Cases & Protectors
 *   7. Power Banks
 *   8. Chargers & Cables (before audio — 3-in-1 docks mention airpods in slug)
 *   9. Audio
 *  10. Accessories (default — boxes, locks, phone lens protectors, etc.)
 */
...
/** Apple Watch cases, screen protectors, and connector adapters — not iPhone lens protectors. */
function isWatchCaseOrProtector(searchStr) {
  if (/\b(iphone|camera lens)\b/i.test(searchStr) && /\blens protector\b/i.test(searchStr)) {
    return false;
  }
  ...
  return isAppleWatchAccessory(searchStr);
}

/** Standalone power banks and MagSafe battery packs — not TWS earbuds with a charging case. */
function isPowerBank(title, searchStr) {
  if (isAudioProduct(searchStr) && /\b(tws|earbuds?|airpods?)\b/i.test(searchStr)) {
    return false;
  }

  if (/\b(power\s*bank|magsafe battery|battery pack|portable battery)\b/i.test(title)) {
    return true;
  }

  return (
    /\b(power\s*bank|magsafe battery|battery pack)\b/i.test(searchStr) && !isAudioProduct(searchStr)
  );
}

/** Chargers, cables, and adapters — not power banks or Apple Watch connector adapters. */
function isChargerOrCable(title, searchStr) {
  if (/\bconnector adapter for apple watch\b/i.test(searchStr)) {
    return false;
  }

  if (isPowerBank(title, searchStr)) {
    return false;
  }

  const titleHasCharger =
    /\b(charger|chargers|charging station|wireless charger|wireless charging|charging cable|adapter kit|data cable|\d+-in-\d+)\b/i.test(
      title
    );
  ...
  return /\b(charger|chargers|charging station|wireless charger|wireless charging|fast charging cable|charging cable|charging adapter|usb[- ]?c cable|lightening cable|lightning cable|data cable|adapter kit|pd adapter|\d+w\s*adapter|\d+\s*pin adapter|foldable.*charg|\d+-in-\d+.*charg)\b/i.test(
    searchStr
  );
}
 ...
 * @returns {string} One of the 10 final categories
 */
function getProductCategory(title = '', tags = '', type = '', handle = '') {
  ...
  // Priority 6: Watch Cases & Protectors
  if (isWatchCaseOrProtector(searchStr)) {
    return 'Watch Cases & Protectors';
  }

  // Priority 7: Power Banks
  if (isPowerBank(safeTitle, searchStr)) {
    return 'Power Banks';
  }

  // Priority 8: Chargers & Cables
  ...
  // Priority 10: Accessories (boxes, locks, phone lens protectors, etc.)
  return 'Accessories';
}
