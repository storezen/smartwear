...
export const LiveEventsFeed = memo(function LiveEventsFeed({ events }: LiveEventsFeedProps) {
      ...
      <div className="bg-card rounded-xl border border-white/5 overflow-hidden">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] tracking-[1.5px] text-foreground/60">LIVE FEED</span>
          </div>
        </div>
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            <div className="w-10 h-10 mx-auto mb-2 rounded-xl bg-card border border-white/5 flex items-center justify-center">
              <Activity className="w-4 h-4 text-foreground/&" />
            </div>
            <p className="text-[11px] text-foreground/&">Awaiting visitors...</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="bg-card rounded-xl border border-white/5 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-white/[0.04]">
        <div className="flex items-center justify-between mb-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] tracking-[1.5px] text-foreground/60">LIVE FEED</span>
            <span className="text-[9px] text-foreground/& font-mono">{stats.total}</span>
          </div>
          <button
        ...
        <div className="flex items-center gap-2">
          {[
            ...
            <div key={s.label} className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-card border border-white/[0.04]">
              <span className={`w-1.5 h-1.5 rounded-full ${s.dot}`} />
              <span className="text-[9px] font-medium text-foreground/40">{s.label}</span>
              <span className="text-[9px] font-mono font-bold text-foreground/70 tabular-nums">{s.count}</span>
            </div>
          ))}

          {/* Latest event preview */}
          {latestMeta && !collapsed && (
            <div className="flex items-center gap-1.5 ml-auto text-[9px] text-foreground/& truncate max-w-[140px]">
      ...
      {/* ── Filter Tabs ── */}
      {!collapsed && (
        <div className="flex items-center gap-0.5 px-4 py-2 border-b border-white/[0.04] bg-card">
          {EVENT_TABS.map(tab => (
            <button
              key={tab.key}
      ...
      {!collapsed && (
        ...
        >
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex items-center gap-2">
                    ...
                    <span className="text-[8px] text-foreground/20 shrink-0 hidden sm:inline">{meta.label}</span>
                  </div>

                  <span className="text-[8px] text-foreground/20 font-mono flex items-center gap-1 shrink-0">
                    <MapPin className="w-2 h-2" />{event.city || "PK"}
                  </span>

                  <span className="text-[8px] text-foreground/& font-mono shrink-0 w-8 text-right">
          ...
          {filtered.length === 0 && (
            <div className="flex items-center justify-center py-8 text-[11px] text-foreground/20">
              No events
            </div>
          )}
        </div>
