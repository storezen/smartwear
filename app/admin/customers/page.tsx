  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight font-playfair mb-2">Customers</h1>
          <p className="text-white/60 text-sm">View and analyze your customer base derived from order history.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="bg-white/5 border border-white/10 text-white rounded-lg pl-9 pr-4 py-2 text-sm focus:outline-none focus:border-[#B8860B] transition-colors w-64"
            />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-white/50">Loading customer data...</div>
      ) : filteredCustomers.length === 0 ? (
        <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-8 text-center mt-12">
          ...
          <p className="text-white/50 text-sm mb-6 max-w-md mx-auto">No customer records match your search criteria.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredCustomers.map((customer, i) => (
            <SpotlightCard key={customer.phone} className="p-5 cursor-pointer hover:border-[#B8860B]/30 transition-colors" onClick={() => openCustomer(customer)}>
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#B8860B] to-[#D4A017] flex items-center justify-center text-white font-bold text-lg">
                    {customer.name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <h3 className="font-bold text-white leading-tight">{customer.name}</h3>
                    <p className="text-xs text-white/40 flex items-center gap-1"><Phone className="w-3 h-3"/> {customer.phone}</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                <div>
                  <p className="text-xs text-white/40 mb-1 flex items-center gap-1"><ShoppingBag className="w-3 h-3"/> Orders</p>
                  <p className="text-lg font-semibold text-white">{customer.total_orders}</p>
                </div>
                <div>
                  <p className="text-xs text-white/40 mb-1 flex items-center gap-1"><DollarSign className="w-3 h-3"/> Lifetime Value</p>
                  <p className="text-lg font-semibold text-[#D4A017]">₨ {customer.total_spent.toLocaleString()}</p>
                </div>
              </div>
            </SpotlightCard>
          ))}
